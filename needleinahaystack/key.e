...
--
--
public function find(object x, integer element, integer start_i=-1, integer end_i=-1, sequence haystack)
...
public function find_any(sequence keys, integer element, integer start_i=-1, integer end_i=-1, sequence haystack)
	if start_i = -1 and end_i = -1 then
		start_i = 1
		end_i = length(haystack)
	elsif start_i = -1 and end_i = 1 then
	...
	if length(haystack)=0 then
		...
		return (eu:find(haystack[start_i][element],keys)!=0)*start_i
	end if
	return 0
end function
